...
### Changed
- **`/review` hands its judgment checks to a fresh-context subagent.** The design-compliance and code-quality checklists now run in a subagent that gets only the PR diff, the plan's acceptance criteria and the project rules, not the implementer's session, PR body, commit messages or gate summary. Every finding it reports goes into the posted verdict; a dismissal has to quote the code that disproves it. Gate verification stays in the session and still re-runs the deterministic gates at the PR HEAD SHA. If no subagent can be spawned, the verdict says `Judgment checks: NOT isolated`.
- **2026-09-24 pipeline-review fixes (pulled from pragma).** `/feature` gains pragma's doubt-driven review step before the GREEN commit, for new protocols (Gate 10) and `@Model` mutations (Gate 9). Its `/loop` stop conditions now list every blocking gate instead of "all 13", because Gates 10 and 12 are advisory. `/bugfix` splits the regular and hotfix paths: both hotfix PRs need `/review`, `/test` and `code-review:code-review`. `/review` reports exit 2 from the TDD script (no file matches `SCOPED_LAYER_DIRS`) as a failure to report, not a pass, and gains an advisory layout-adaptivity check. Gate 1 links pragma's Xcode 27 SDK-migration note. Gate 2 and the golden-test counter also match Swift Testing's `✔`/`✘` lines, like `/test`; Xcode 27 prints `Test case '…' passed` for this repo's Swift Testing suites, so the count does not change (191 on the 2026-09-24 run). The pseudo-path `AGENTS.md/CLAUDE.md` is replaced with `AGENTS.md` in 39 places, and `/review`'s stale `gates.md`/`gates.yml` references now name `gates/SKILL.md` and the `gates` job in `pr-checks.yml`. Eight one-off session entries moved from `.claude/settings.json` to the gitignored `settings.local.json`.
...
- **The pipeline-review alert never reached the agent.** It used only `systemMessage`, which Claude Code shows to the user but does not add to the agent's context, so "ask the user whether to address these findings" was never acted on. It now also sends `hookSpecificOutput.additionalContext`, and gets the report name with `sed` instead of `xargs basename`, which failed on a name containing a quote.
- **Gate 2, the golden-test counter, `/test` and `/release` counted Swift Testing's run summary (`✔ Test run with N tests … passed`) as a passed test**, so a run that executed nothing counted as 1. The summary line is now excluded.
- **`/release`'s force-unwrap check flagged `if !flag` and missed `let x = y!`.** It now matches an identifier or closing bracket followed by `!`.
- **`/review` described exit 2 from the TDD script as "template default"**, pragma's meaning. In this repo it means no file matches `SCOPED_LAYER_DIRS` (folders renamed or moved).
- **The PR creation rule said `hotfix/*` never uses `--base develop`**, which contradicted `/bugfix`'s second PR back to `develop`. AGENTS.md and `/gates` now name that exception.
...
### Changed
- **Guard hook hardened, and gate-integrity check 1 now covers every file the hook protects.** Ported from pragma PR #77 after two review rounds. The hook now inspects `git rm`/`git mv`, subshells, `if`/`for`/`while` bodies, `eval`, backticks and quoted `$( )`, `bash -lc`, `xargs`/`nice`/`timeout` wrappers, backslash-newline continuations, partly quoted words and `cp`/`install`/`ln`/`mv` into a protected directory. A trailing redirect no longer hides a destination, a protected name that is a symlink is caught, removing the repo root or an ancestor is blocked, and paths with `+`, `@` or non-ASCII characters resolve correctly. Self-test grows from 52 to 174 checks and a new `gates` job step in `pr-checks.yml` runs it, so the hook's and the script's glob lists cannot drift silently. `check_gate_integrity.py` check 1 now flags any skill, `AGENTS.md`, `CLAUDE.md`, `.claude/settings.json`, `.claude/hooks/*`, `scripts/check_*` and `ImportHashGoldenTests.swift` edit on a `feature/*` PR (new `GUARDED_PATH_GLOBS`), matched at any depth, including a guarded file renamed away. The `pr-checks.yml` `paths:` filter gains the same files, each also as `**/…` so a PR touching only a nested one still triggers the job. A `feature/*` PR that edits `.claude/settings.json` now fails Gate 13, so move permission-list edits to a `chore/*` branch.

### Fixed
- **Pipeline-review alert and `/status` counted a report as unaddressed when its body merely mentioned `addressed: false`.** Both ran `grep -rl 'addressed: false'` over each whole file, so `docs/pipeline-review/2026-08-15.md` (frontmatter `addressed: true`, but line 28 describes the hook in prose) triggered the `UserPromptSubmit` alert on every prompt. Both now read only the frontmatter's `addressed:` line. Checked against all 6 existing reports (0 flagged), a test report with `addressed: false` in its frontmatter (flagged), and a file with the phrase only in its body (not flagged).
- **`/test` and `/release`'s pre-flight piped `xcodebuild` to `xcsift`, which hides its exit status** — both now write a log file and require `xcodebuild`'s own exit code, the `TEST SUCCEEDED` marker and an executed-test count above zero, the same pattern `/gates` Gate 2 uses; `sync-workflow.md` now calls Gate 0 the "build-relevant change check" to match its widened trigger list.
- **Xcode 27 / Swift 6.4 build break on `develop`: `FakeTransactionImportWriting` and `FakeCategorySuggesting` (both `actor`s) no longer compiled** — Swift 6.4 infers `@MainActor` onto `TransactionImportWriting` and `CategorySuggesting` via the project's `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`, and an actor cannot conform to a global-actor-isolated protocol (no per-conformance annotation works around it; only un-isolating the protocol does). This blocked Gate 1 for every PR built under local Xcode 27. Both protocols are now `nonisolated`, gated `#if compiler(>=6.4)` because CI's pinned Xcode 26.4 / Swift 6.2 doesn't recognise `nonisolated` on a protocol declaration (and never had the bug) — its `#else` branch is the unchanged declaration. CI still only compiles the `#else` branch; the `nonisolated` branch is exercised on local Xcode 27 only. New invariant #6 records the pattern for any future actor-conformed protocol (`FoundationModelsCategorySuggester.isAvailable`'s isolation now differs by toolchain, noted in a comment there).
- **Gate 12's "Gate 2 already left a simulator booted" premise was wrong** — verified on Xcode 27 that `xcodebuild test` tears its simulator down on completion (single-suite and full runs), so `boot_sim` is the normal path, not a fallback. Gate 10's new-protocol scan now also matches `nonisolated protocol` and other modifier/attribute prefixes, so protocols written per invariant #6 aren't skipped, and it no longer reports invariant #6's intentional `#if`/`#else` duplication as an extraction candidate.

## [1.3.0] — 2026-09-09

### Fixed
- **`BudgetListView` and `ImportSheet` silently swallowed throws via `try?`, across every call site in both files, not just the original `Task {}` ones** — `BudgetListView`'s month-change reload, `onAppear`, and swipe-to-delete, plus `ImportSheet`'s "Parse & Preview" and `onAppear`, all produced no error and no user-visible signal on failure. Added `markLoadFailed()`/`dismissLoadFailure()` on `BudgetViewModel` (shared by all three `BudgetListView` call sites via one `.alert`) and two new `ImportFailure` cases (`.mappingFailed`, `.loadFailed`) + `markMappingFailed()`/`markLoadFailed()` on `ImportViewModel`, wired into the existing alert. All underlying `load()`/`delete()`/`applyMapping()` calls still throw unchanged, so existing callers and tests are unaffected. Also fixed `ImportSheet` running a full on-device category-suggestion pass over stale `pendingTransactions` after a failed mapping (moved `loadSuggestions()` inside the success path), and a `BudgetViewModel.load()` partial-mutation risk where `categories` could be published while `budgets`/`unbudgetedCategories` (derived from it) stayed stale if a later fetch in the same call threw — `load()` now only publishes state after every fetch succeeds. Caught via the 2026-09-08 iOS Engineering Intelligence Report's `Task {}` silent-throw audit, then extended by `code-review:code-review`'s correctness-verify and cleanup passes on the resulting PR.
- **`pipeline-review.md`'s valid-skills registry was missing `benchmark`, `parallel-review`, and `pr-followup`**, all three actively self-invoked or cross-referenced elsewhere in the pipeline — added to the registry. Ported from pragma's 2026-09-07 `/pipeline-review` fix (pragma PR #55).
- **`pr-followup.md`, `parallel-review.md`, `gates.md`, `feature.md`, `plan.md`, `test.md`, and `CLAUDE.md` documented `code-review:code-review` as manual-only / not agent-invokable**, dated to a 2026-07-30 confirmation (PRs #67-70) that has since been superseded — `feedback_pr_gate_execution` memory (confirmed on this repo's own PR #100, 2026-08-19) already established it runs automatically, and this session re-confirmed it directly by invoking the skill six times against pragma. Updated all seven files so `code-review:code-review` auto-chains alongside `/review` and `/test` by default, with the `disable-model-invocation` exception documented once, canonically, in `pr-followup.md`. `parallel-review.md`'s Check 1 also no longer points at `review.md`'s post-`/gates`-only Architecture section (unsatisfiable pre-PR) — it now runs Gate 9 directly, same fix pattern as pragma. Ported from pragma PR #55 after its own 5-round code-review convergence; applied here in the already-converged form.
...
### Added
- **Ported 4 pragma pipeline improvements developed upstream this session** — `/spec`/`/plan` now fetch Apple docs as Markdown (append `.md` to any `developer.apple.com` URL) instead of HTML; `/bugfix`/`/review` now read/write `.claude/context/incidents.md`, a searchable bug root-cause log mirroring `decisions.md`/`rejections.md`/`feature-log.md`; new `/benchmark` command + `scripts/capture_pipeline_metrics.py` for comparing pipeline changes against a repeatable baseline; new `scripts/slim_simulator.sh` (optional, no-ops without the `simslim` CLI installed) wired into all 4 simulator-boot points across `pr-checks.yml`/`ui-tests.yml`/`release.yml`, measured on this repo's own test suite at ~3x memory reduction with zero test breakage. A 5th candidate (adding `-enableCodeCoverage YES` to `release.yml`, needed in pragma's generic template) was checked and does NOT apply here — verified empirically that this repo's scheme already collects real coverage without the flag.
