...
- **Gate 12's "Gate 2 already left a simulator booted" premise was wrong** — verified on Xcode 27 that `xcodebuild test` tears its simulator down on completion (single-suite and full runs), so `boot_sim` is the normal path, not a fallback. Gate 12 now also runs only once every blocking gate has passed, so the autonomous `/loop` no longer reboots the simulator each iteration. Gate 10's new-protocol scan now also matches `nonisolated protocol`, so protocols written per invariant #6 aren't skipped.
