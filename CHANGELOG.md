...
- `/trim-context` pass: trimmed `CLAUDE.md`'s Architecture section (dropped a one-line MVVM/Repository description that just restated folder structure already visible from browsing the project; the enforced layer-rule bullets below convey the same layers through their content) — 51 lines → 49, back under the 50-line budget. Curated `.claude/settings.json`'s permission allowlist: removed 23 entries already fully covered by two pre-existing blanket wildcards (`git -C .../FinanceTracker *` and `git -C .../pragma *`), removed ~50 more dead one-off entries (a stale `ios-agent-workflow` path that no longer exists, pid/window-ID-specific `cua-driver` calls from an old LinkedIn screenshot session, duplicate PIL-availability diagnostic checks, one-off carousel/slide-deck file opens, a stale scratchpad path from a different session), consolidated the `cua-driver` pid-specific calls into 3 general wildcards, and fixed one stale `additionalDirectories` entry (`Wiki/LinkedIn Posts` → the actual current path `Wiki/LinkedIn Posts Wiki`). Net: 220 → 149 allowlist entries
- Fix `/parallel-review`'s docs claiming it runs `code-review:code-review` "in parallel" as an automated subagent task — confirmed 2026-07-30 that skill has `disable-model-invocation` set and isn't in the agent-invocable skill list at all, so no agent-driven path can trigger it. The architecture-checklist check still runs automatically; the command now prompts the user to run `code-review:code-review` themselves alongside it
- Add `/pr-followup` command — auto-chains `/review` then `/test` right after a PR opens, the two pipeline stages that don't need a human trigger. `code-review:code-review` is deliberately excluded: confirmed 2026-07-30 that it has `disable-model-invocation` set and doesn't appear in the agent-invocable skill list at all, so no agent-driven path can trigger it — `/pr-followup` prints an explicit reminder to run it manually instead. `/gates` now points to it as the next step after opening a PR
- Fix two `/gates` false positives found while running the pipeline on PRs #67-#70: Gate 4's branch-naming pattern list was missing `chore/*` despite it being an established convention in this repo's history; Gate 9's ViewModels-concrete-repo check matched any path containing `/ViewModels/`, including `FinanceTrackerTests/ViewModels/`, flagging test files that legitimately construct concrete SwiftData repos per CLAUDE.md's own in-memory test pattern — now excludes `Tests/` like the existing `try!`/`as!` check does
