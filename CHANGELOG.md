...
- **The pipeline-review alert never reached the agent.** It used only `systemMessage`, which Claude Code shows to the user but does not add to the agent's context, so "ask the user whether to address these findings" was never acted on. It now also sends `hookSpecificOutput.additionalContext`.
