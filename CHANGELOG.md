...
- `BudgetViewModel` gains a `categorySuggester` dependency (defaulted), `suggestionsAvailable`, and `canOpenAddBudget`; `AddBudgetSheet`'s empty state is now device-aware — CSV-import-only messaging when Apple Intelligence suggestions are available, a manual "Add Category" fallback (presenting `AddCategorySheet` inline) when they aren't — and `BudgetListView`'s toolbar Add button is enabled whenever that fallback is reachable
