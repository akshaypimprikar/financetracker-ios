...
- **Pipeline: `/feature` now commits RED and GREEN separately, and `/gates` Gate 11 verifies the order** — a git-history audit of three merged feature branches found every ViewModel task commit bundled the failing test and its implementation together, so "write a failing test first" was unverifiable from outside the agent's own narration (the exact failure mode documented in Martin Fowler's "TDD inside the agent loop" and corroborated independently by Simon Willison and testdouble). `/feature` also now invokes the already-installed `test-driven-development` Superpowers skill instead of a one-line paraphrase; `scripts/check_tdd_commit_order.py` does the actual verification, checking the branch as it stands rather than requiring history rewrites
