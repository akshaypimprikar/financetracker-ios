...
### Fixed
- **`BudgetListView` and `ImportSheet` silently swallowed `Task {}` throws via `try?`** — a failed `BudgetViewModel.load()` after a month change (or on appear) and a failed `ImportViewModel.applyMapping()` (most commonly `existingHashes()` failing) both produced no error and no user-visible signal. Added `markLoadFailed()`/`dismissLoadFailure()` on `BudgetViewModel` and a new `.mappingFailed` `ImportFailure` case + `markMappingFailed()` on `ImportViewModel`, each surfaced via a new/extended `.alert`; both view models' underlying `load()`/`applyMapping()` still throw unchanged, so existing callers and tests are unaffected. Caught via the 2026-09-08 iOS Engineering Intelligence Report's `Task {}` silent-throw audit.
