...
### Fixed
- **`/test` piped `xcodebuild` to `xcsift`, which hides its exit status** — it now writes a log file and checks `xcodebuild`'s own exit code and the `TEST SUCCEEDED` marker, the same pattern `/gates` Gates 1-2 use; `sync-workflow.md` now calls Gate 0 the "build-relevant change check" to match its widened trigger list.
