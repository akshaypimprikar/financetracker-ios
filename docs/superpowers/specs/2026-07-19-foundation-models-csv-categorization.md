...
- **Manual/on-device verification:** required before `/release` — the real `FoundationModelsCategorySuggester` must be exercised on a physical A17 Pro+/M1+ device (or eligible simulator) with a real CSV to confirm the guided-text `categoryName` match behaves as expected in practice, since this is the one part of the design that extrapolates beyond the pre-verified research.
