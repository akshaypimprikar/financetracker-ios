...
**Cross-repo rule:** never use `cd` for ios-agent-workflow operations — the shell working directory persists across tool calls and silently affects subsequent `gh`/`git` commands. Always use `git -C /Users/akshaypimprikar/Desktop/Claude/ios-agent-workflow <cmd>` and `gh pr create --repo akshaypimprikar/ios-agent-workflow`.
