...
**Merge rule:** a PR is eligible to merge once `/review` returns APPROVED, `/test` passes, and `code-review:code-review` is clean. Approval must come from the user's actual GitHub PR review — check `gh pr view <N> --json reviewDecision -q .reviewDecision`; a chat "yes" is not sufficient and no command asks for one. Once GitHub shows `APPROVED` and all three checks are clean, merge automatically (`gh pr merge <N> --merge`, matching this repo's merge-commit convention) and run the standard post-merge cleanup (pull the base branch, delete the local branch, prune remote refs) without being asked.
