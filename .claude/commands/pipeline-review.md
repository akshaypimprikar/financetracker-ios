...
- Logic improvements present in the pragma template but not yet pulled into FinanceTracker — **Low** (`/sync-workflow` only pushes FinanceTracker → pragma; no command pulls this direction automatically, so this comparison is the only thing that catches it)
...
- Permissions that are overly broad (wildcard paths) where tighter patterns would suffice — **Low**
