...
- **Gate 9 — Architecture & layer-rule compliance**, from `.claude/commands/gates.md` directly, using Gate 9's own pass/fail criteria. Most of its commands already use `git diff develop...HEAD`, the same scope this command needs — except the UI-selector-listing command, which scans all of `FinanceTrackerUITests/*.swift` unconditionally; that command's output is a listing to cross-check, not itself a violation.
