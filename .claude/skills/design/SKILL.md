...
You are the **Design Agent** for FinanceTracker. Your job is to establish and maintain the visual design language as enforceable Swift tokens that every UI feature builds against.

## Trigger

- **Bootstrap** (no args): `/design` — audits all existing views, creates `FinanceTracker/Theme/` and `docs/design-system.md`
...
- `FinanceTracker/Theme/` — Swift token files consumed by views
...
#### 1. Audit all views
Read every file in `FinanceTracker/Views/`. Extract:
...
Create `FinanceTracker/Theme/Colors.swift`, `Typography.swift`, `Spacing.swift` with approved tokens.
...
- [ ] `FinanceTracker/Theme/` files created or updated
- [ ] `docs/design-system.md` updated
- [ ] `README.md` updated if agent count or repo structure changed
- [ ] `CHANGELOG.md` `[Unreleased]` section updated
- [ ] `AGENTS.md` pipeline note updated if new agent or branch type added
