...
Read `AGENTS.md` and flag any content that violates these rules:
...
| Non-obvious domain constraints | Navigation structure (visible in ContentView) |
...
- Redundant with current project state (e.g., a "we use main not develop" memory after AGENTS.md already says so)
...
- Duplicated verbatim in AGENTS.md or a command file

Report findings. Only edit skills if clearly outdated — skills load on-demand so verbosity matters less than AGENTS.md.

### 5. Runtime read/tool-output waste (diagnostic only, no proxy required)
If `headroom` is installed (`command -v headroom`), run:
```bash
headroom audit-reads --path ~/.claude/projects/-Users-akshaypimprikar-Desktop-Claude --format json
```
`--path` scopes it to the transcripts that actually matter here — its default with no `--path` is `~/.claude/projects`, every tracked project's transcripts combined. Do **not** derive this path from `pwd` (e.g. via `sed 's/[^A-Za-z0-9]/-/g'`) — Claude Code sessions for this project launch from the `~/Desktop/Claude` root, not from `FinanceTracker/` itself (see `project_root_launch_migration` memory), so a `pwd`-derived path resolves to the old per-project directory, which is now a stale, unmaintained backup rather than where this project's live transcripts land. It streams the scoped transcripts read-only and sizes addressable waste by category (identical repeats, stale reads, line-number scaffolding) as structured fields (`dedup_identical_bytes`, `stale_bytes`, `linenum_overhead_bytes`, etc.) — read those directly rather than parsing text output. This is a diagnostic, not a compression layer: it doesn't touch anything, it just tells you where the waste actually is.
...
End with the new line count for AGENTS.md.
...
- Commit any changes to AGENTS.md and settings.json with message `chore: trim context after <plan-name>`

## Done when
AGENTS.md is ≤50 lines, stale memory entries removed, settings.json allow list tightened, and changes committed.
